...
test_plain_text_turn() {
  s=$(hnew)
  hsh -q ask "$s" 'hello there' >/dev/null
  assert_contains "$(hsh show "$s")" '[mock] You said: hello there'
}

test_tool_call_turn_runs_and_records_output() {
  s=$(hnew)
  hsh -q ask "$s" 'run [[tool:bash:echo loopmarker]]' >/dev/null
  res=$(hsh assemble "$s" | jq -r '[.[].content[]|select(.type=="tool_result")][0]')
  assert_contains "$res" 'loopmarker'
  assert_eq "$(printf '%s' "$res" | jq -r '.is_error')" 'false' 'tool not flagged as error'
}

test_wire_format_alternates_and_groups() {
  s=$(hnew)
  hsh -q ask "$s" 'go [[tool:bash:echo x]]' >/dev/null
  assert_eq "$(hsh assemble "$s" | jq -r '[.[].role]|join(",")')" \
            'user,assistant,user,assistant' 'roles alternate'
  # the tool-calling assistant message carries text + tool_use
  assert_eq "$(hsh assemble "$s" | jq -c '.[1].content|map(.type)')" '["text","tool_use"]'
  # the following user message carries the tool_result
  assert_eq "$(hsh assemble "$s" | jq -c '.[2].content|map(.type)')" '["tool_result"]'
}
...
test_manifest_has_columns() {
  s=$(hnew)
  hsh -q send "$s" 'hi'
  line=$(hsh manifest "$s" | head -1)
  assert_contains "$line" '0001,user,text,'
  assert_contains "$line" ',ok'
}
