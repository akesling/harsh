...
test_write_then_read_with_offset() {
  d=$(mktemp -d); f="$d/x.txt"
  tool write "$(jq -nc --arg p "$f" '{path:$p,content:"a\nb\nc\n"}')" >/dev/null
  out=$(tool read "$(jq -nc --arg p "$f" '{path:$p,offset:2,limit:1}')")
...
test_edit_nonunique_is_error() {
  d=$(mktemp -d); f="$d/x.txt"; printf 'b\nb\n' > "$f"
  assert_fails tool edit "$(jq -nc --arg p "$f" '{path:$p,old:"b",new:"c"}')"
  rm -rf "$d"
}

test_edit_all_replaces_every_occurrence() {
  d=$(mktemp -d); f="$d/x.txt"; printf 'b\nb\n' > "$f"
  tool edit "$(jq -nc --arg p "$f" '{path:$p,old:"b",new:"c",all:true}')" >/dev/null
  assert_not_contains "$(cat "$f")" 'b'
  rm -rf "$d"
}

test_edit_literal_special_chars() {
  d=$(mktemp -d); f="$d/x.txt"; printf 'one\n' > "$f"
  tool edit "$(jq -nc --arg p "$f" '{path:$p,old:"one",new:"a/b&c.*"}')" >/dev/null
  assert_contains "$(cat "$f")" 'a/b&c.*'
  rm -rf "$d"
}
...
test_every_tool_schema_is_valid() {
  for t in "$ROOT"/tools/*.sh; do
    b=$(basename "$t" .sh); [ "$b" = tool ] && continue
    sh "$t" --schema | jq -e '.name and .input_schema.type=="object"' >/dev/null \
      || fail "invalid schema: $b"
  done
}
